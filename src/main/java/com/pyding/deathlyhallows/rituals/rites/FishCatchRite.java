...
public class FishCatchRite extends ElderRite {
	private final float upkeepPowerCost;
	private final int ticksToLive;
	private final int radius;

	public FishCatchRite(int radius, float upkeepPowerCost, int ticksToLive) {
		this.radius = radius;
	...
	private static class StepVanish extends ElderRitualStep {
		private final FishCatchRite rite;
		...
		public StepVanish(FishCatchRite rite, int ticksSoFar) {
			super(false);
			this.rite = rite;
				...
				if(!world.isRemote) {
					...
					while(i$.hasNext()) {
						...
						if(Coord.distance(player.posX, player.posY, player.posZ, posX, posY, posZ) <= (double)r) {
							...
							for(int i = 0; i < 20; i++) {
								ItemStack itemStack = FishingHooks.getRandomFishable(random, 10, 99, 10);
