...
public class CommandDamageLog extends CommandBase {
	@Override
		...
		if(args.length > 0 && args[0].equalsIgnoreCase("damagelog")) {
			...
			if(sender instanceof EntityPlayer) {
				...
				try {
					...
					DHUtils.getFigure(player.worldObj, (int)player.posX + x, (int)player.posY + y, (int)player.posZ + z, xSize, ySize, zSize);
