...
public class EntityAbsoluteDeath extends EntityMob implements IBossDisplayData, IHandleDT {
	...
	public float baseDamage = 33 * DHConfig.deathDifficulty;

	public static EntityPlayer mvp;
	public float bestDamage;

	public EntityAbsoluteDeath(World par1World) {
		super(par1World);
...
/*            if(o instanceof EntityThrowable)
		...
		if(super.worldObj.rand.nextDouble() < 0.05D && this.getAttackTarget() != null && (this.getAttackTarget().isAirBorne || this.getAttackTarget() instanceof EntityPlayer && ((EntityPlayer)this.getAttackTarget()).capabilities.isFlying) && !this.getAttackTarget()
	...
	protected boolean teleportTo(double par1, double par3, double par5) {
		...
		if(super.worldObj.blockExists(i, j, k)) {
			...
			if(short1) {
				...
				if(super.worldObj.getCollidingBoundingBoxes(this, super.boundingBox)
								 .isEmpty() && !super.worldObj.isAnyLiquid(super.boundingBox)) {
					flag = true;
				}
			}
		}
	...
	public void onDeath(DamageSource par1DamageSource) {
		...
		dropDeathlyHallows();
		this.dropItem(DHItems.deathShard, 1);
	...
	public void dropDeathlyHallows() {
		List id = null;
		List entities = this.worldObj.getEntitiesWithinAABB(EntityPlayer.class, this.getBoundingBox()
																					.expand(80, 80, 80));
		for(Object o: entities) {
			EntityPlayer player = (EntityPlayer)o;
		...
		if(mvp != null) {
			...
			if(random < 0.33) {
				this.dropItem(DHItems.elderWand, 1);
			}
			else if(random < 0.66) {
				this.dropItem(DHItems.invisibilityMantle, 1);
			}
			else {
				this.dropItem(DHItems.resurrectionStone, 1);
			}
		}
	}
